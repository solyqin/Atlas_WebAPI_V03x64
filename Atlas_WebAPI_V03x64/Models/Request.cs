...
namespace Atlas_WebAPI_V03x64.Models
{
    ...
    public class Request_param
    {
        ...
        public void GetParam(HttpContext context)
        {
            ...
            string filepath = context.Request.Form["file"];
            ...
            try
            {
                ...
                if (string.IsNullOrEmpty(filepath))
                    throw new MyException("缺少识别文件路径！");
                if (!File.Exists(filepath))
                    throw new MyException($"本地文件不存在！:{filepath}");
    ...
    public class Report_param
    {
        //private string[] files { set; get; }
        //public string[] Files { get { return this.files; } }
        private ReportModel reportParam { set; get; }
        public ReportModel ReportParam { get { return this.reportParam; } }

        public Report_param(HttpContext context)
        {
            GetReportParam(context);
        }
        private void GetReportParam(HttpContext context)
        {
            //IFormFileCollection filelist = context.Request.Form.Files; //上传文件
            //this.files = Request_param.SaveFile(filelist);
            
            string param = context.Request.Form["Parameter"];
            this.reportParam = JsonConvert.DeserializeObject<ReportModel>(param);
        }
    }
}